...
@section('title', $title ?? 'University-Materials')
@section('content')
                                ...
                                @if(count($list)>0)
                                    @foreach ($list as $item)
                                        <div class="col-md-4 col-12 h-25 text-center">
                                            <a @if((App\Models\UG_PG\Material::where('university_name',$item->name_slug)->count()>0)) href="{{ug_pg('materials/'.$item->name_slug.'/ug-undergraduate')}}" @else href="#"   data-toggle="modal" data-target="#info_popup" @endif   class="btn-btn-info text-light">
                                                <div class="card text-white  min-height-100 bg1 mb-3 w-100" >
                                                <h3 class="card-body mt-3   text-uppercase text-light h6">{{remove_slug($item->name_slug)}}</h3>
