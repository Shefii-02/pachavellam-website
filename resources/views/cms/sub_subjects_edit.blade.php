@extends('cms.section')
        ...
        <div class="card-header bg-transparent border-bottom">
            <div class="row">
                ...
                <div class="col-md-7 text-end">
                    ...
                    <a class="btn btn-sm btn-primary me-1" href="{{route('adminkpsc.sub-subjects.index')}}" ><i class="bi bi-left-arrow me-1"></i>Back to</a>
  ...
  {{-- @can('Psc Subjects Add')--}} 
                    <form action="{{route('adminkpsc.sub-subjects.update',$subject->id)}}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-md-5">
                                <div class="text-center justify-content-center align-items-center p-4 p-sm-5 border border-2 border-dashed position-relative rounded-3">
                                    <!-- Image -->
                                    <img src="{{ Storage::url('files/'.$subject->image) }}" id="uploaded-image" class="uploaded-image h-50px mb-2" alt="">
                                    <div>
                            ...
                            <div class="col-md-5">
                                ...
                                <div class="form-group mb-2">
                                    ...
                                    <input class="form-control" required type="color" value="{{$subject->text_color}}" value="#ffffff" name="text_color"  >
                                </div>
                                
                                <div class="form-group mb-2">
                                    <label class="form-label">Parent Subject</label>
                                    <select class="form-control" name="parent">
                                        @foreach($parent_subjects as $itm)
                                        <option value="{{$itm->id}}" @if($subject->parent_id == $itm->id) selected @endif>{{$itm->subject_title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                 <div class="form-group mb-2">
                                            ...
                                            <div class="col-6 mb-2">
                                                <div class="form-check-inline">
                                                  <label class="form-check-label">
                                                    <input type="checkbox" class="form-check-input"  name="activity[]"  @if($count_val > 0) checked @endif value="{{$item->id}}"> {{$item->activity}}
                                                  </label>
                                                </div>
                                            </div>
                                        @endforeach
