             ...
             <div class="card-body">
                 <table class="table table-dark-gray table-bordered align-middle p-4 mb-0 table-hover">
                     <tr>
                         ...
                         <th scope="col" class="border">{{ $item->examtitle }}</th>
                     ...
                     <tr>
                         <th colspan="4">
                             <a href="{{ kpsc_cms('daily-exam/edit/' . $item->id) }}"
                             ...
                             <a target="_new"
                                 href="{{ url('kpsc/daily-exam/' . $item->exam_date . '/' . $item->id . '/leaderboard') }}"
                                 class="btn btn-sm btn-primary mb-2 float-left">
                                 <i class="bi bi-bar-chart"></i>
                                 View Leaderboard
                             </a>

                             <a target="_new2"
                                 href="{{ url('kpsc/daily-exam/' . $item->exam_date . '/' . $item->id) }}"
                             ...
                             <a onclick="return confirm('Are you sure can`t retrive data?')"
                                 href="{{ kpsc_cms('daily-exam/delete-all/' . $item->id) }}"
                                 class="btn btn-danger btn-sm mb-2 float-left"><i class="fa fa-trash"></i> Delete Exam
                                 With
                                 All Questions</a>

                             <a onclick="return confirm('Are you sure can`t retrive data?')"
                                 href="{{ kpsc_cms('daily-exam/clear-leaderboard/' . $item->id) }}"
