                ...
                <div class="card bg-light rounded">
                  <div class="card-body">
                        <table class="table table-dark-gray table-bordered align-middle p-4 mb-0 table-hover">
                            ...
                            <tr>
                                <th colspan="4">
                                    <a  href="#"  data-questionpaper="{{ url(Storage::url('model-exam/'.$date_based->qp_file)) }}" data-answerkey="{{ url(Storage::url('model-exam/'.$date_based->answer_file)) }}"   data-date="{{$date_list->exam_date}}"  data-started="{{date('H:i',strtotime($date_list->started_at))}}" data-id="{{$date_list->id}}"  data-ended="{{date('H:i',strtotime($date_list->ended_at))}}" data-title="{{$date_list->examtitle}}" data-subject="{{$date_list->subject}}" class="btn btn-info mb-2 exam_details_edit btn-sm float-left">
