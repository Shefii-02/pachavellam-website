...
@section('content')
  ...
  @endsection
    @section('scripts')
                            <script>
                            let PDFFILE="{{ url(Storage::url('model-exam/'.$examdetails->qp_file)) }}";
            ...
            {{--    var dateVal = "{{date('M d, Y',strtotime($examdetails->exam_date))}}";
                var timeVal = "{{date('h:i a',strtotime($examdetails->ended_at))}}";
        @if(date('Y-m-d H:i:s', strtotime($examdetails->ended_at))  > date('Y-m-d H:i:s'))

            $(document).ready(function() {
                // Set the date we're counting down to
                var countDownDate = new Date("{{ date('Y-m-d H:i:s', strtotime($examdetails->ended_at)) }}").getTime();
                ...
                var x = setInterval(function() {
                  ...
                  if (distance < 0) {
                      clearInterval(x);
                      if (confirm("Are you done with the exam? Answer key is available.")) {
                          window.location = "/kpsc/model-exam";
                      } else {
                          window.location = "";
                      }
                  }
